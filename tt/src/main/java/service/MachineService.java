...
public class MachineService implements IDao<Machine> {
	...
	public boolean create(Machine o) {
		...
		try {
			...
			ps.setInt(4, o.getIdMarque().getId());
	...
	public Machine findById(int id) {
		Machine m = null;
		MarqueService mm = new MarqueService();
		...
		try {
			...
			if (rs.next()) {
				return new Machine(rs.getInt("id"), rs.getString("reference"), rs.getDate("dateAchat"),
						rs.getDouble("prix"), mm.findById(rs.getInt("idMarque")));
			}
	...
	public List<Machine> findMachineByMarque(int idM) {
		...
		MarqueService mm = new MarqueService();
		String sql = "select * from machine where idMarque = ? order by idMarque desc";
...
//	public List<Machine> ReturnMachineByMarque(int idM) {
//
//		List<Machine> machines = new ArrayList<Machine>();
//
//		MarqueService mm = new MarqueService();
//		String sql = "select count(*) from machine where idMarque = ? order by idMarque desc";
//
//		try {
//
//			PreparedStatement ps = Connexion.getInstane().getConnection().prepareStatement(sql);
...
//	}
	...
	public List<Machine> findCountMachineByMarque() {
		List<Machine> machines = new ArrayList<Machine>();
		MarqueService mm = new MarqueService();
		String sql = "select m.*,count(*) as Nbm from machine m inner join marque ma on m.idMarque=ma.id   group by m.id";
		try {
			PreparedStatement ps = Connexion.getInstane().getConnection().prepareStatement(sql);
			...
			while (rs.next()) {
				machines.add(new Machine(rs.getInt("id"), rs.getString("reference"), rs.getDate("dateAchat"),
						rs.getDouble("prix"), mm.findById(rs.getInt("idMarque")), rs.getInt("nbM")));
			}
	...
	public List<Machine> findAll3() {
		...
		try {
			...
			while (rs.next()) {
				System.out.println("a1 " + rs.getInt("id"));
				System.out.println("a2 " + rs.getString("reference"));
				System.out.println("a3 " + ms.findById(rs.getInt("idMarque")));
				System.out.println("a4 " + rs.getInt("count"));
				machines.add(new Machine(rs.getInt("id"), rs.getString("reference"), ms.findById(rs.getInt("idMarque")),
						rs.getInt("count")));
		...
		System.out.println("a1" + machines);
		return machines;
	...
	public int countMachine() {
		...
		System.out.println("jjjjj" + count);
		return count;
